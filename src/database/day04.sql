...
SELECT mid, sum( bamount ) 총구매수량 from buy GROUP BY mid; -- 1) mid기준으로 그룹하여 총 구매수량(bamount)
...
# SELECT mid , sum( bamount ) 총구매수량 FROM
...
# [6] limit: 결과 레코드 제한, *메이징처리*
SELECT * from member;   -- 10개
SELECT * from member LIMIT 2;   --1 ~ 2개 (2)
SELECT * from member LIMIT 0, 2;    -- 1 ~ 2개 (0번부터 2개) (2)
SELECT * from member LIMIT 5, 5;    -- 5번부터 5개 (5)

-- [순서] select 필드명 from 테이블명 where 조건절 group by 그룹조건 having 그룹조건 order by 정렬필드 limit 시작인덱스, 개수
