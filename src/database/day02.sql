...
CREATE Table test1( 필드명1 int, 필드명2 int, 필드명3 text );
# 2) 테이블 목록 조회
SHOW TABLE;
# 3) 특정 테이블 속성 확인
DESCRIBE test1;
...
# 3) 필드/속성명, 타입 수정
ALTER Table test1 CHANGE 필드명1 필드명 5 BIGINT;
...
CREATE Table test2( 
    ...
    정수필드4 INT  , 정수필드5 BIGINT   , int UNSIGNED
    -- unsigned 부호없다. tinyint(1바이트 -128 ~ 127) --> 0 ~ 225
    상수필드1 FLOAT ,상수필드2 DOUBLE ,상수필드3 DECIMAL
    -- DECIMAL 문자타입 이면서 소수점 오차 없음
    날짜필드1 DATE ,    시간필드1 TIME,    날짜시간필드 DATETIME,
    문자필드1 CHAR(3).  문자필드2 VARCHAR,  문자필드3 TEXT, 문자필드4 longtext.
