...
public class Exam1 {
    public static void main(String[] args) {
        // 예외란? 에러 발생 시 고치는게 아니라 흐름 제어 vs if 
        // 예외처리 : try{ 예외발생코드 }catch( 예외클래스명 변수명 ){ 처리할코드 }

        // [1] 일반예외 : 실행(컴파일)하기 전 예외가 발생할 수 있는 코드에 대해 미리 예외처리
        // 1. ClassNotFoundException : 클래스 찾을 수 없다
        try{
            Class.forName("string"); // Class.forName("찾을클래스") 동적으로 존재하는 클래스 로드
        }catch( ClassNotFoundException e ){
            System.out.println("존재하지 않은 클래스 입니다." + e);
        }
        // 2. InterruptedException : 스레드에 문제가 발생하면 
        try{
            Thread.sleep(1000); // Thread.sleep( 밀리초 ); 밀리초만큼 일시정지, 스레드란? 컴파일(번역)된
            }catch( InterruptedException e){ // 예외 발생시 처리되는 코드, e : 변수이면서 예외정보 담는객체
                System.out.println("인터럽트 문제 발생11"+e);
            }
            // [2] 실행예외 : 실행( 컴파일 ) 후 예외 발생 *경험 중요* 예측 하거나 유지보수( 로그 )
            // NullPointerExceptionullpointer  : 참조가 없는데 참조하는 경우
            try{
            String str1 = null;  // null 이란? 참조값이 없다 즉] 객체(인스턴스)가 없다.
            System.out.println( str1.length() );// .(도트/접근) 연산자는 참조할 대상의 멤버들 결정
