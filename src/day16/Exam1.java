...
public class Exam1 {
    public static void main(String[] args) {
        ...
        /*
            스레드 : 코드 실행 흐름 단위,
            멀티스레드ㅣ 여러 사용자들이 하나의 웹서버(톰캣)에 요청함으로 웹서는 멀티스레드로 관리
            장점 : 여러 요청들을 다중처리, 단점: 관리(동기화/과부화)
            1. 동기화 vs 비동기화
                동기화 : 락(대기)상태 이용하여 단 *하나의 스레드*가 하나의 메소드 점유
                    - 무결성 보장함, 순서가 필요한 경우: ,
                비동기화 : *여러 스레드*가 하나의 메소드같이 점유
...
class 스레드2 extends Thread{
    public 계산기 calculator;
    @Override public void run() {
        calculator.setMemory(200); // 계산기 메소드에 200 대압
    }
}

class 계산기{ // 클래스란? 인스턴스 설계도(종이/ 실체 아니다)
    // 멤버변수란? 인스턴스의 상태/변수/값 저장소/필드/속성, 인스턴스마다!! 할당
    public int memory;
    // 메소드란? 인스턴스의 행위/ 이벤트, 여러 인스턴스가 공유/같이

    // [1] 비동기화(기본값)     : 200(두번째요청이 출력 전 변경함) 200 출력, SNS( 출력, DM, 영상 )
    // public void setMemory( int memory ){
    // [2] 동기화(synchronized) : 100(두번째요청이 대기상태) 200 출력
    public synchronized void setMemory( int memory ){
        this.memory = memory; //this란? 현재 메소드를 호출한 인스턴스 가리킴, 매개변수명과 구분용
